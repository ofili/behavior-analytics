...
if [[ $# -eq 0 ]]; then
    echo "Enter your bucket name as ./setup_aws.sh your-bucket"
    exit 0
fi
...
echo "Reading infrastructure variables from environment (.env) file"
source .env

echo "Creating bucket "$1" in AWS"
...
aws iam create-role --role-name $EC2_IAM_ROLE --assume-role-policy-document file://policy.json --description "EC2 access to S3" --output text >> setup.log

echo "Attaching AmazonS3FullAccess Policy to the IAM role"
aws iam attach-role-policy --role-name $EC2_IAM_ROLE --policy-arn arn:aws:iam::$AWS_ID:policy/AmazonS3FullAccess --output text >> setup.log

echo "Attaching AmazonEMRFullAccessPolicy_v2 to the IAM role"
aws iam attach-role-policy --role-name $EC2_IAM_ROLE --policy-arn arn:aws:iam::$AWS_ID:policy/AmazonEMRFullAccess_v2 --output text >> setup.log

echo "Attaching AmazonRedshiftAllCommandsFullAccessPolicy to the IAM role"
aws iam attach-role-policy --role-name $EC2_IAM_ROLE --policy-arn arn:aws:iam::$AWS_ID:policy/AmazonRedshiftAllCommandsFullAccessPolicy --output text >> setup.log

echo "Creating  IAM instance profile to add to EC2"
aws iam create-instance-profile --instance-profile-name $EC2_IAM_ROLE --output text >> setup.log
aws iam add-role-to-instance-profile --instance-profile-name $EC2_IAM_ROLE --role-name $EC2_IAM_ROLE --output text >> setup.log

echo "Creating ssh key to connect to the EC2  instance
aws ec2 create-key-pair --key-name sde-key --query "KeyMaterial" --output text --region #    > sde-key.pem
...
EC2_SECURITY_GROUP_ID=$(aws ec2 create-security-group --description "Security group to allow inbound SCP connections" --group-name $EC2_SECURITY_GROUP --output text)
echo 'EC2_SECURITY_GROUP_ID="'$EC2_SECURITY_GROUP_ID'"' >> state.log


echo "Add inbound rule to all ssh from IP $MY_IP"
...
echo "Add outbound rule to allow IP $MY_IP to connect to EC2's port 8080"
...
echo " Get EC2 ID"
sleep 20
EC@_ID=$(aws --region $AWS_REGION ec2 describe-instances --filters "Name=instance-state-name,Values=running" "Name=tag:Name,Values=$AWS_EC2_INSTANCE_NAME" --query 'Reservations[*].Instances[*].[InstanceId]' --output text)
...
do
        ...
        sleep 60
        EC2_STATUS=$(aws ec2 describe-instance-status --instance-ids $EC2_ID --query 'InstanceStatustes[0].InstanceState.Name' --output text)
...
echo "SCP to copy to remote server"
cd ../
scp -o "IdentitiesOnly yes" -i ./behavior-analytics/sde-key.pem -r ./behavior-analytics ubuntu@$EC2_IPV4:/home/ubuntu/behavior-analytics
cd behavior-analytics

echo "Clean up old data"
sleep 10
ssh -o "IdentitiesOnly yes" -i "sde-key.pem" ubuntu@$EC2_IPV4 'cd behavior-analytics && rm -f data.zip && rm -rt data'

echo " Download data"
sleep 10
ssh -o "IdentitiesOnly yes" -i  "sde-key.pem" ubuntu@$EC2_IPV4 'cd behavior-analytics && wget https://start-data-engg.s3.amazonaws.com/data.zip && sudo unzip data.zip && sudo chmod 755 data' 

echo "Recreate logs and temp dir"
sleep 10
ssh -o "IdentitiesOnly yes" -i "sde-key.pem" ubuntu@$EC2_IPV4 'cd behavior-analytics && rm -rf logs && mkdir logs && rm -rf temp && mkdir temp && chmod 755 temp'

echo "Creating and AWS EMR  Cluster with name "$SERVEICE_NAME""
aws emr create-default-roles >> setup.log
aws emr create-cluster --applications Name=Hadoop Name=Spark --release-label emr-6.2.0 --name $SERVEICE_NAME --scale-down-behavior TERMINATE_AT_TASK_COMPLETION --service-role EMR_DefaultRole --instance-groups '[
        {
            "InstanceCount" : 1,
            "EbsConfiguration": {
            ...
            "InstanceGroupType": "'$EMR_NODE_TYPE'",
            "Name": "Master - 1"
        },
        {
            "InstanceCount": 2,
...
aws iam create-role --role-name $EC2_IAM_ROLE --assume-role-policy-document file://trust-policy.json --description 'spectrum access for redshift' >> setup.log

echo "Attaching  AmazonS3ReadOnlyAccess Policy to the IAM role"
aws iam attach-role-policy --role-name $IAM_ROLE_NAME --policy-arn arn:aws:iam::$AWS_ID:policy/AmazonS3ReadOnlyAccess --output text >> setup.log
echo "Attaching AWSGlueConsoleFullAccess Policy to to IAM role"
aws iam attach-role-policy --role-name $IAM_ROLE_NAME --policy-arn arn:aws:iam::$AWS_ID:policy/AWSGlueConsoleFullAccess --output text >> setup.log

echo "Creating an AWS Redshift Cluster named "$SERVEICE_NAME""
aws redshift create-cluster --cluster-identifier $SERVEICE_NAME --node-type dc2.large --master-username $REDSHIFT_USER --master-user-password $REDSHIFT_PASSWORD --cluuster-type single-node --publicly-accessible --iam-roles "arn:aws:iam::"$AWS_ID":role/"$IAM_ROLE_NAME"" >> setup.log

while:
do
        echo "Waiting  for Redshift cluster "$SERVEICE_NAME" to start, sleeping for 60s before next check"
        sleep 60'
        REDSHIFT_CLUSTER_STATUS=$(aws redshift describe-clusters --cluster-identifier $SERVEICE_NAME --query 'Clusters[0].ClusterStatus' --output text)
...
REDSHIFT_HOST=$(aws redshift describe-clusters --cluster-identifier $SERVEICE_NAME --query 'Clusters[0].Endpoint.Address' --output text)
